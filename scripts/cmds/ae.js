...
module.exports = {
  ...
  onChat: async function ({ api, event, args, message }) {
    try {
      ...
      if (!prefix) {
        return; // Ignore messages without specified prefixes
      }
      
      const prompt = event.body.substring(prefix.length).trim();
      
      if (prompt === '') {
        await message.reply("Enter your question, dude. Anya AI is waiting.");
        return;
      }
      
      await message.reply("Waiting for Anya AI's response...");
